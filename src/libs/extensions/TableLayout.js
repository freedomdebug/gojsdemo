...
TableLayout.prototype.getEffectiveTableStretch = function(child, row, col) {
  ...
  if (
    ...
    (str === go.GraphObject.Horizontal || str === go.GraphObject.Fill)
  ) {
    horizontal = true;
  } else {
    horizontal = false;
  ...
  if (
    vertical === undefined &&
    (str === go.GraphObject.Vertical || str === go.GraphObject.Fill)
  ) {
    vertical = true;
  } else {
    vertical = false;
 ...
 * @this {TableLayout}
 */
TableLayout.prototype.measureTable = function(
  width,
  height,
  children,
  union,
  ...
  for (var i = 0; i < lrow; i++) {
    ...
    for (var j = 0; j < lcol; j++) {
      ...
      for (var k = 0; k < len; k++) {
        ...
        var stretch = this.getEffectiveTableStretch(
          child,
          rowHerald,
          colHerald
        );
  ...
  for (var i = 0; i < l; i++) {
    ...
    switch (stretch) {
      case go.GraphObject.Horizontal: // H stretch means it can be as large as its wants vertically
        allowedSize.height = Math.max(
          allowedSize.height,
          rowHerald.actual + rowleft
        );
        break;
      case go.GraphObject.Vertical: // vice versa
        allowedSize.width = Math.max(
          allowedSize.width,
          colHerald.actual + colleft
        );
        break;
    }
    ...
    switch (stretch) {
      case go.GraphObject.Fill:
        if (actualSizeColumns[colHerald.index] !== 0) {
          allowedSize.width = Math.min(
            allowedSize.width,
            actualSizeColumns[colHerald.index]
          );
        }
        if (actualSizeRows[rowHerald.index] !== 0) {
          allowedSize.height = Math.min(
            allowedSize.height,
            actualSizeRows[rowHerald.index]
          );
        }
        break;
      case go.GraphObject.Horizontal:
  ...
  for (var i = 0; i < lrow; i++) {
    ...
    for (var j = 0; j < lcol; j++) {
      ...
      for (var k = 0; k < len; k++) {
        ...
        ar.x +=
          ar.width * alignx - widthmarg * alignx + alignoffsetX + marg.left;
        ar.y +=
          ar.height * aligny - heightmarg * aligny + alignoffsetY + marg.top;
